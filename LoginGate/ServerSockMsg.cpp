...
DWORD WINAPI AcceptThread(LPVOID lpParameter)
{
	...
	while (TRUE)
	{
		...
		char str[32] = "server has recive";
		send(Accept,str,32,0);
	...
	while (TRUE)
	{
		...
		while ( pSessionInfo->HasCompletionPacket() )
		{
			...
			*pszPos		= '\0';
			...
			TCHAR log[100];
			//WideCharToMultiByte(CP_ACP,0,szPacket,sizeof(szPacket),log,100,NULL,NULL);

			MultiByteToWideChar(CP_ACP,0,szPacket,-1,log,100);

			//memcpy(log,szPacket,100);
