...
void OnCommand(WPARAM wParam, LPARAM lParam)
{
	switch (LOWORD(wParam))
	{
		case IDM_STARTSERVICE:
		{
			...
			char str[32] = "192.168.0.100";

			char lpStr[20];
			MultiByteToWideChar(CP_ACP, 0, str, sizeof(str), (LPWSTR)lpStr, sizeof(lpStr));
			//strcpy(str,"192.168.0.100");
			//ConnectToServer(g_csock, &g_caddr, _IDM_CLIENTSOCK_MSG, (LPCTSTR)str, dwIP, nPort, FD_CONNECT|FD_READ|FD_CLOSE);
			ConnectToServer(g_csock, &g_caddr, _IDM_CLIENTSOCK_MSG, (LPWSTR)lpStr, dwIP, nPort, FD_CONNECT|FD_READ|FD_CLOSE);
