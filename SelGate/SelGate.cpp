...
BOOL InitInstance(HANDLE hInstance, int nCmdShow)
{
	...
	LV_COLUMN nwzvc;
	nwzvc.mask = LVCF_FMT | LVCF_WIDTH | LVCF_TEXT | LVCF_SUBITEM;
	nwzvc.fmt		= LVCFMT_LEFT;
	nwzvc.cx		= 200;
	nwzvc.pszText = sTmp;

	ListView_InsertColumn(g_hLogMsgWnd,3,&nwzvc);

  */

	ListView_SetColumnWidth( g_hLogMsgWnd, 1, 200 );
	ListView_SetColumnWidth( g_hLogMsgWnd, 2, 200 );
	//ListView_SetColumnWidth( g_hLogMsgWnd, 3, 200 );
	ListView_SetColumnWidth( g_hLogMsgWnd, 0, 200 );
